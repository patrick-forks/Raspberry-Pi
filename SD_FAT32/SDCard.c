...
SDRESULT sdClearBlocks(uint32_t startBlock, uint32_t numBlocks) {
    ...
    uint32_t endAddress = sdCard.type == SD_TYPE_2_SC ? (uint32_t)((startBlock + numBlocks) << 9) : (uint32_t)(
...
static bool LoadDrivePartition(printhandler prn_basic) {
    ...
    if ((bpb->FATSize16 == 0) && (bpb->RootEntryCount == 0)) {        // Check if FAT16/FAT32
        ...
        if (prn_basic)
            prn_basic("FAT32 Volume Label: %s, ID: %08x\n",
        ...
        if (bpb->FSTypeData.fat1612.BS_BootSig == 0x29) {
            if (prn_basic)
                prn_basic("FAT12/16 Volume Label: %s, Volume ID %08x\n",
    ...
    if (prn_basic)
        prn_basic("First Sector: %lu, Data Sectors: %lu, TotalClusters: %lu, RootCluster: %lu\n",
...
uint32_t getSetNextCluster(uint32_t clusterNumber, bool set, uint32_t clusterEntry) {
    ...
    FATEntrySector = sdCard.partition.unusedSectors + sdCard.partition.reservedSectorCount +
                     ((clusterNumber * 4) /
                      sdCard.partition.bytesPerSector);    // Get sector number of the cluster entry in the FAT
    FATEntryOffset = ((clusterNumber * 4) %
                      sdCard.partition.bytesPerSector); // Get the offset address in that sector number
    if (sdTransferBlocks(FATEntrySector, 1, (uint8_t * ) & buffer[0], false) != SD_OK) {
        return 0xFFFFFFFF;                                            // Sector read failed twice so fail exit
    }
    FATEntryValue = (uint32_t * ) &
                    buffer[FATEntryOffset];                // Get the cluster address from the buffer .. fortunately always aligned
...
static struct dir_Structure *
LocateFATEntry(const char *searchPat, struct PRIV_SEARCH_DATA *priv, bool dirChange, LFN_NAME LFN_Name,
               uint32_t *ErrorID) {
...
HANDLE sdFindFirstFile(const char *lpFileName, FIND_DATA *lpFFData) {
    ...
    CopyUnAlignedString(&tempName[0],
                        lpFileName);                    // Copy the name string as we will chop and change it
...
HANDLE sdFindNextFile(HANDLE hFindFile, FIND_DATA *lpFFData) {
    ...
    if ((hFindFile > 0) && (hFindFile <= MAX_SEARCH_RECORDS) &&
        (intSearchRecord[hFindFile - 1].firstSector !=
         0)) {            // File handle maps to an internal search record in use
        ...
        intSearchRecord[hFindFile -
                        1].firstSector = 0;                // Clear first sector which is used to mark in use
    }
    return 0;                                                        // Return the handle and its error
}
                    ...
                    HANDLE hTemplateFile)                            // Currently not supported (use 0)
{
    ...
    CopyUnAlignedString(&tempName[0],
                        lpFileName);                    // Make local copy of name as we will chop and change it
    const char *searchStr = &tempName[0];                            // Search string starts as lpFilename
    if (searchStr[0] == '\\')
                ...
                void *lpOverlapped)                                // Currently not supported (use 0)
{
    if ((hFile > 0) && (hFile <= MAX_FILE_IO_RECORDS) &&
        (intFileIORecord[hFile - 1].srec.firstSector !=
         0)) {        // File handle maps to an internal file io record in use
        ...
        while ((retErrId == FAT_RESULT_OK) && (intFileIORecord[hFile - 1].srec.cluster < 0x0ffffff6)
            ...
            {
                ((uint8_t *) lpBuffer)[bytesRead] = intFileIORecord[hFile - 1].srec.buffer[intFileIORecord[hFile -
                                                                                                           1].srec.bPos++]; // Transfer byte and increment pointer
            ...
            if (intFileIORecord[hFile - 1].filePos >= intFileIORecord[hFile -
                                                                      1].fileSize) {                                                        // File end reached
                if (lpNumberOfBytesRead) *lpNumberOfBytesRead = bytesRead;
...
SDRESULT sdInitCard(printhandler prn_basic, printhandler prn_error, bool mount) {
    ...
    if (prn_basic)
        prn_basic("EMMC: SD Card %s %dMb mfr %d '%c%c:%c%c%c%c%c' r%d.%d %d/%d, #%08x RCA %04x\n",
                  SD_TYPE_NAME[sdCard.type], (int) (sdCard.CardCapacity >> 20),
                  sdCard.cid.MID, sdCard.cid.OID_Hi, sdCard.cid.OID_Lo,
                  sdCard.cid.ProdName1, sdCard.cid.ProdName2, sdCard.cid.ProdName3, sdCard.cid.ProdName4,
                  sdCard.cid.ProdName5,
                  sdCard.cid.ProdRevHi, sdCard.cid.ProdRevLo, sdCard.cid.ManufactureMonth,
                  2000 + sdCard.cid.ManufactureYear, serial,
                  sdCard.rca >> 16);
